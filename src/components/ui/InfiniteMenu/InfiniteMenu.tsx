...
class InfiniteGridMenu {
  ...
  private animate(deltaTime: number): void {
    ...
    // Track the camera-facing (front) disc so only the main item is visible when idle.
    let frontIndex = 0;
    let maxZ = -Infinity;
    ...
    this.activeInstanceIndex = frontIndex;

    // Smoothly ramp how many discs are visible so nothing pops in/out while
    // spinning or switching items: 0 = only the active disc, 1 = all discs.
    const targetReveal = this.movementActive ? 1 : 0;
    this.revealFactor += (targetReveal - this.revealFactor) * 0.1;
    if (Math.abs(this.revealFactor - targetReveal) < 0.005) this.revealFactor = targetReveal;

    positions.forEach((p, ndx) => {
      const s = (Math.abs(p[2]) / this.SPHERE_RADIUS) * SCALE_INTENSITY + (1 - SCALE_INTENSITY);
      // Hide every disc except the active one while idle; reveal all while spinning.
      let finalScale = s * scale;
      if (ndx !== this.activeInstanceIndex) {
        finalScale = s * scale * Math.max(0.0001, this.revealFactor);
      }
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown) {
      // Active item = the camera-facing disc (matches the only visible circle).
      const itemIndex = this.activeInstanceIndex % Math.max(1, this.items.length);
      this.onActiveItemChange(itemIndex);
      const snapDirection = vec3.normalize(vec3.create(), this.getVertexWorldPosition(this.activeInstanceIndex));
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      try {
        ...
        // WebGL2/shader failure should degrade the menu only, never the whole page.
        console.error('InfiniteMenu failed to initialise:', err);
        setInitFailed(true);
      }
    }
  ...
  const openModal = () => {
    if (activeItem) setIsModalOpen(true);
  };
  ...
  if (initFailed) {
    return (
      <div className="h-full w-full overflow-y-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
        {(items.length ? items : defaultItems).map((item, i) => (
          <div key={i} className="rounded-xl border border-white/10 bg-white/[0.04] overflow-hidden">
            ...
            <div className="p-4">
              ...
              <p className="mt-2 text-xs leading-relaxed text-white/80 line-clamp-4">{item.description}</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      {activeItem && !isModalOpen && (
        ...
        `}
          >
            {activeItem.description.length > 220
              ? activeItem.description.slice(0, 220) + '…'
      ...
      {isModalOpen && activeItem && (
        ...
        >
          <div
            className="relative w-full max-w-lg max-h-[82%] overflow-y-auto rounded-2xl border border-white/10 bg-white/[0.07] shadow-2xl"
          ...
          >
            ...
            <img src={activeItem.image} alt={activeItem.title} className="h-44 w-full object-cover" />

            <div className="p-6">
              <h3 className="text-xl font-bold leading-snug text-white">{activeItem.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-white/85">{activeItem.description}</p>
