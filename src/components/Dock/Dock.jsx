...
function DockItem({
  ...
  return (
    <motion.div
      ...
      className={`relative inline-flex items-center justify-center rounded-full bg-[#120F17]/80 border border-white/15 shadow-md backdrop-blur-md ${className}`}
      tabIndex={0}
      role="button"
      aria-haspopup="true"
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute -top-6 left-1/2 w-fit whitespace-pre rounded-md border border-white/15 bg-[#120F17]/90 px-2 py-0.5 text-xs text-white`}
          role="tooltip"
          style={{ x: '-50%' }}
        >
          {children}
