...
export const StaggeredMenu = ({
  ...
  // Controlled mode: when `open` is provided, the parent owns the state.
  const isControlled = open !== undefined;
  const [openState, setOpenState] = useState(false);
  const effectiveOpen = isControlled ? open : openState;
  ...
  const setMenuState = useCallback(
    (target, notify = true) => {
      ...
      if (!isControlled) setOpenState(target);
      if (target) {
        onMenuOpen?.();
        playOpen();
      } else {
      ...
      animateColor(target);
      animateText(target);
      if (notify && isControlled) onOpenChange?.(target);
    },
  ...
  // Follow external (controlled) open changes.
  useEffect(() => {
    if (!isControlled) return;
    if (open !== openRef.current) {
      setMenuState(open, false);
    }
  }, [open, isControlled, setMenuState]);

  useEffect(() => {
    if (!closeOnClickAway || !effectiveOpen) return;
  ...
  }, [closeOnClickAway, effectiveOpen, closeMenu]);
